...
def _wait_for_underlay_readiness(config: TopologyConfig) -> None:
    ...
    raise RuntimeError(
        ...
        "Start sdwan/scripts/run_controller.sh first and inspect " + str(path)
    )
...
def _start_workloads(nodes: Mapping[str, Any], plan: LiveTopologyPlan, config: TopologyConfig) -> None:
    ...
    _run_checked(nodes[config.data_center_app_name], [
        "sh", "-c",
        "setsid nohup python3 -m uvicorn sdwan.workloads.backup_service:app "
        ...
        "pgrep -f 'uvicorn sdwan.workloads.backup_service:app'",
    ])
    _run_checked(nodes[config.saas_app_name], [
        "sh", "-c",
        "setsid nohup python3 -m uvicorn sdwan.workloads.public_saas_service:app "
        ...
        "sleep 0.4; pgrep -f 'uvicorn sdwan.workloads.public_saas_service:app'; "
            ...
            if node.persistent_identity:
                parameters.update({
                    "volumes": [
                        ...
                        f"{config.source.resolve()}:/opt/sdwan/config/topology.yaml:ro",
                    ],

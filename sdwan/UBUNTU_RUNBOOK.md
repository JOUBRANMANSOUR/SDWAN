...
PYTHONPATH=$PWD python sdwan/scripts/initialize_trust.py \
  --state-root /mnt/data/sdwan-state
PYTHONPATH=$PWD python -m sdwan.persistence.migrate \
...
```bash
bash sdwan/scripts/build_images.sh
```
...
cd /mnt/data/sdwan-lab
source ~/ryu-venv38/bin/activate
PYTHONPATH=$PWD python sdwan/scripts/live_enroll.py \
  --state-root /mnt/data/sdwan-state \
...
bash sdwan/scripts/run_management.sh
```
...
```text
node2_host sh -c '/opt/sdwan/workloads/rtp_receiver.sh /opt/sdwan/workloads/rtp-video.sdp /tmp/received-video.mkv > /tmp/rtp-receiver.log 2>&1 &'
```

Start the sender:

```text
node1_host sh -c '/opt/sdwan/workloads/rtp_sender.sh 10.2.0.10 5004 > /tmp/rtp-sender.log 2>&1 &'
```
...
```text
node1_host python3 /opt/sdwan/workloads/backup_client.py node1 --size-mib 64
```
...
```text
node1_host python3 /opt/sdwan/workloads/saas_client.py interactive --requests 30 --interval 0.2
```
...
```bash
bash sdwan/scripts/failure_injection.sh <scenario>
```

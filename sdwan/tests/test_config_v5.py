...
from sdwan.edge_agent_v5 import EdgeAgent
from sdwan.topology_visualization import render_dot, render_files
from sdwan.common.marks import EgressMode, FlowMarkStage
...
class ConfigAndMarkTests(unittest.TestCase):
    ...
    def test_host_image_packages_the_concurrent_workload_client(self) -> None:
        ...
        self.assertIn("COPY sdwan/workloads/http_load.py /opt/sdwan/workloads/http_load.py", dockerfile)
    ...
    def test_classifier_events_are_collected_as_bounded_metadata_only_jsonl(self) -> None:
        ...
        self.assertIn("sdwan.classifier_event_collector", agent)
    ...
    def test_classifier_event_collector_strips_packet_content(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            process = subprocess.Popen(
                [sys.executable, "-m", "sdwan.classifier_event_collector", "--socket", str(socket_path), "--output", str(output)],
                env=environment,
            )
            try:
    ...
    def test_graphviz_renderer_has_a_readable_logical_view_and_exact_physical_view(self) -> None:
        ...
        self.assertEqual(logical.count(" -- "), 24)
        self.assertEqual(physical.count(" -- "), 46)
        for name in ("hub1", "hub2", "node1", "node5", "s_mpls", "s_bb", "s_lte", "dc_app", "inet_gw", "public_saas"):

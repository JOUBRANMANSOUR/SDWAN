...
```bash
cd /mnt/data/sdwan-lab
sudo docker build -f sdwan/docker/Dockerfile.edge.v5 \
  -t containernet-sdwan-edge-v5:latest .
sudo docker build -f sdwan/docker/Dockerfile.host.v5 \
...
```bash
cd /mnt/data/sdwan-lab
bash sdwan/scripts/run_controller.sh
```
...
SDWAN_STATE_ROOT=/mnt/data/sdwan-state \
  bash sdwan/scripts/run_ztp_service.sh
```
...
SDWAN_DESTINATION_POLICY_CONFIG=$PWD/sdwan/config/destination_policy.yaml \
  bash sdwan/scripts/run_policy_service.sh
```
...
sudo -E env PATH="$PATH" PYTHONPATH="$PWD" \
  "$VIRTUAL_ENV/bin/python" sdwan/scripts/live_enroll.py \
  --state-root /mnt/data/sdwan-state \
  --inventory "$PWD/sdwan/config/site_inventory.yaml"
```
...
node1_host python3 /opt/sdwan/workloads/backup_client.py node1 --size-mib 8
node1_host python3 /opt/sdwan/workloads/saas_client.py interactive --requests 5 --interval 0.1
node1_host python3 /opt/sdwan/workloads/saas_client.py upload --file /tmp/saas-document.bin
node1_host python3 /opt/sdwan/workloads/saas_client.py download --file /tmp/saas-document.bin
```
...
```text
node2_host sh -c '/opt/sdwan/workloads/rtp_receiver.sh /opt/sdwan/workloads/rtp-video.sdp /tmp/received-video.mkv >/tmp/rtp-receiver.log 2>&1 & echo $!'
node1_host sh -c '/opt/sdwan/workloads/rtp_sender.sh 10.2.0.10 5004 >/tmp/rtp-sender.log 2>&1 & echo $!'
```
...
```bash
bash sdwan/scripts/failure_injection.sh degrade-node1-mpls
```
...
```bash
bash sdwan/scripts/failure_injection.sh restore-node1-mpls
```
...
```bash
bash sdwan/scripts/failure_injection.sh congest-node1-bb
```
...
```bash
bash sdwan/scripts/failure_injection.sh stop-congestion
```
...
```bash
bash sdwan/scripts/failure_injection.sh hub1-down
```
...
node1 cat /var/lib/sdwan/state/path-decisions.json
node1 cat /var/lib/sdwan/state/path-events.json
node1_host python3 /opt/sdwan/workloads/saas_client.py interactive --requests 5
```
...
```bash
bash sdwan/scripts/failure_injection.sh hub1-up
```
...
```bash
bash sdwan/scripts/failure_injection.sh broadband-down
```
...
```bash
bash sdwan/scripts/failure_injection.sh broadband-up
```
...
```bash
SDWAN_TOPOLOGY_CONFIG=$PWD/sdwan/config/topology.cloud.yaml \
SDWAN_DESTINATION_POLICY_CONFIG=$PWD/sdwan/config/destination_policy.cloud.yaml \
SDWAN_STATE_ROOT=/mnt/data/sdwan-state \
  bash sdwan/scripts/run_policy_service.sh
